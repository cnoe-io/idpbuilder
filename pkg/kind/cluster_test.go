...
- |-
  ...
  [plugins."io.containerd.grpc.v1.cri".registry.configs."cnoe.localtest.me".tls]
	...
	for i := range tcs {
		c := tcs[i]
		cluster, err := NewCluster("testcase", "v1.26.3", "", "", "", []string{}, v1alpha1.BuildCustomizationSpec{
			Host:           c.host,
			Port:           c.port,
...
func TestExtraPortMappings(t *testing.T) {
	...
	cluster, err := NewCluster("testcase", "v1.26.3", "", "", "22:32222", []string{}, v1alpha1.BuildCustomizationSpec{
		Host: "cnoe.localtest.me",
		Port: "8443",
	}, logr.Discard())
	if err != nil {
...
func TestGetConfigCustom(t *testing.T) {
	...
	for _, v := range cases {
		c, _ := NewCluster("testcase", "v1.26.3", "", v.inputPath, "", []string{}, v1alpha1.BuildCustomizationSpec{
			Host:     "cnoe.localtest.me",
			Port:     v.hostPort,
			Protocol: v.protocol,
		}, logr.Discard())
